...
exports.bookinstance_delete_get = function(req, res) {
    async.parallel({
        book_instance: function(callback) {
          BookInstance.find({ '_id': req.params.id }).exec(callback)
        },
        ...
        // Successful, so render.
         res.render('bookinstance_delete', { title: 'Delete Book Instance', id: results.book_instance[0]._id, book_instance: results.book_instance } );
    });
};
...
exports.bookinstance_delete_post = function(req, res, next) { 
    ...
    async.parallel({
        book_instance: function(callback) { 
            BookInstance.find({ '_id': req.body.bookinstanceid }).exec(callback); 
          },
    }, function(err, results) {
        if (err) { return next(err); }
        // Success
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                res.render('bookinstance_form', { title: 'Update Book Instance', bookinstance: results.bookinstance, book_list: results.book_list, errors: errors.array() });
            });
            return;
        }
        else {
